...
本文档说明如何将 OpenClaw 的 channel 插件适配到 squid 上运行。

## 背景

OpenClaw 是一个成熟的多渠道 AI 助手平台，拥有丰富的 channel 插件生态（飞书、钉钉、Telegram、Discord 等）。squid 提供了一个最小化的适配层，让这些插件能够在 squid 上运行。
...
- 实现插件实际使用的接口  
- 提供合理的默认值或降级方案  
- 不实现全部 OpenClaw 接口（按需扩展，避免一次性全量对齐）  
...
## 飞书：Adapter 入站 API 与 EventBridge（内置实现）

squid 内置 **飞书开放平台直连**（`FeishuChannelPlugin`），不依赖 OpenClaw `@openclaw/feishu` 运行时。入站消息的**唯一**投递入口为：
...
| Payload 形状 | `ChannelInboundEvent`（见 `src/channels/bridge/event-bridge.ts`），含 `channelId: 'feishu'`、`timestamp` |
...
若将来采用 **OpenClaw 兼容 shim**，shim **必须**将插件侧入站转发到 `submitFeishuInboundToEventBridge`（或同等封装），以满足 `feishu-openclaw-compatibility` spec。

兼容性分析见 [COMPATIBILITY.md](./COMPATIBILITY.md)。
