...
更细的界面与流程说明见 **[QUICK_START.md](./QUICK_START.md)** 与 **[user-guide.md](./user-guide.md)**。  
需要多语言阅读（中/英/日/俄/意/法/德）可从 **[文档入口](../index.html)** 进入统一文档入口并切换语言。
...
squid 采用 **Electrobun**：主进程与本地服务由 Bun 侧运行，界面在系统 WebView 中展示。若你在**克隆的 squid 仓库根目录**下开发并需要加载仓库自带的渠道扩展，可设置环境变量 **`SQUID_ROOT`** 指向该仓库根（使程序能发现其中的 `config/channel-extensions.json`）；日常用户从安装包运行时无需关心此项。模块划分、扩展与工具约定见 **[developer-guide.md](./developer-guide.md)** 与 **[tool-development-guide.md](./tool-development-guide.md)**。
...
| 文档 | 适合谁 |
|------|--------|
| [QUICK_START.md](./QUICK_START.md) | 想尽快跑通功能 |
| [user-guide.md](./user-guide.md) | 想系统了解各菜单与能力 |
| [developer-guide.md](./developer-guide.md) | 参与开发与扩展 |
| [tool-development-guide.md](./tool-development-guide.md) | 编写或修改内置工具 |
| [RELEASE_NOTES.md](./RELEASE_NOTES.md) | 版本变更说明 |
| [TEST_REPORT.md](./TEST_REPORT.md) | 测试与质量说明 |

英文或其它语言原文仍位于仓库 `docs/` 与根目录 `README.md`（与本页对应）。
