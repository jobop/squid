...
本文档定义了 squid 中工具的实现规范，确保所有工具遵循统一的模式，优化上下文使用和用户体验。
...
- **claude-code-main 实现**（与本仓库并列时）：`../../../../claude-code-main/src/utils/toolResultStorage.ts`
- **Tool 接口定义**：`src/tools/base.ts`
- **示例工具**：`src/tools/read-file.ts`、`src/tools/grep.ts`

若 monorepo 布局不同，请按实际路径调整上述 `claude-code-main` 引用。
